...
beforeEach(() => {
  dir = mkdtempSync(join(tmpdir(), "posteria-ledger-telemetry-"));
});
...
describe("telemetry — default-off reachability", () => {
  it("never invokes the telemetry seam when enable_anon_telemetry is false (default)", async () => {
    ...
    for (let i = 0; i < 25; i++) ledger.record(action());
    await ledger.close();
...
describe("telemetry — opt-in reachability (wired)", () => {
  it("invokes the telemetry seam exactly once per record() when enabled", async () => {
    const path = join(dir, "audit.jsonl");
    const spy = mock.fn();
    const ledger = createLedger(
...
describe("telemetry — no network side effects with the real stub", () => {
  it("opens no sockets and issues no DNS/HTTP/HTTPS requests across record()+close()", async () => {
